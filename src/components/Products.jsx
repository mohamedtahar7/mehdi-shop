...
const Products = () => {
  ...
  const currentProducts = products.slice(firstProductIndex,lastProductIndex) || products.slice(firstProductIndex,products.length+1) 
  return (
    <section>
        <Header/>
        <div  className='h-auto py-40 px-20'>
        <h1 className='sm:text-5xl text-3xl text-[#11334f] text-center'>Nos Produits</h1>
        <div className='flex sm:flex-row flex-col justify-center gap-4 mt-6 items-center'>
          ...
          <div  className='flex md:flex-row flex-col sm:w-auto w-fit gap-4 items-center'>
            {categories.map((type,index)=>(<p onClick={()=>{setCategory(type);setCurrentPage(1)}} key={index} className={`text-xl hover:text-white hover:bg-[#11334f] transition-all cursor-pointer ${category===type&&'text-white bg-[#11334f]'} px-4 transition-all border border-[#11334f]`}>{type}</p>))}
          </div>
        </div>
        {
        category==='Tout' && (
