...
const CheckoutContact = ({ sucessOrder, setSuccessOrder }) => {
  ...
  const cartItems = `\n${cart.map(
    (item, index) => `
  Produit ${index + 1} : ${item.name} , Price : ${item.price} DA , Quantity : ${
      item.amount
    }, Color:${item.color} |
  `
  )}`;
  ...
  return (
    <form
      ...
      ref={form}
      onSubmit={sendEmail}
      className="flex flex-col gap-6 md:w-[70%] w-full"
    >
      <div className="flex flex-col gap-6 md:w-[80%] w-full">
        <div className="flex flex-col gap-4">
          ...
          <input
            ...
            name="client_name"
            type="text"
            className="py-2 px-4 text-xl placeholder-gray-400 border focus:border-[2px] border-[#11334f]"
        ...
        <div className="flex flex-col gap-4">
          <label className="text-[#11334f] text-xl font-medium">
            Numéro de Téléphone
          </label>
          <input
            required
      ...
      <button
        type="submit"
        className="text-center py-2 px-4 bg-[#11334f] text-xl hover:opacity-80 transition-all text-white w-fit"
      >
        Commander
      </button>
    </form>
