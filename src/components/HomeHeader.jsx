    ...
    <nav className={`py-4 z-10 ${activeNav?'bg-[#11334f]' : 'bg-none'} transition-all sm:px-16 px-6 fixed w-full flex justify-between items-center`}>
        ...
        <div className='lg:block hidden'>
         <div>
            <ul className='flex items-center gap-12'>
                ...
                <li className={`text-2xl ${!activeNav?'hover:text-[#11334f]':''} transition font-normal text-[#fff] `}><Link to='/products'>Produits</Link></li>
        ...
        <div className={`absolute ${mobileNav?'top-28':'-top-96'} right-7 transition-all duration-300 bg-[#11334f] py-8 px-6`}>
            <ul className='flex flex-col gap-8 items-center'>
                ...
                <li onClick={()=>setMobileNav(false)} className='text-2xl transition font-medium text-[#fff] '><Link className='cursor-pointer' to='/products'>Produits</Link></li>
            </ul>
        </div>
    </nav>
    </div>
