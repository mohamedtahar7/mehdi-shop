    ...
    <div>
      ...
      <div className='py-[16rem] px-16 flex lg:flex-row flex-col justify-center gap-4 items-center'>
        ...
        <div className='flex relative flex-col gap-6'>
            {shownImg===1?<img src={product.images[1].image} alt="image" className='w-[100%] rounded-lg'/> : <img src={product.images[0].image} alt="image" className='w-[80%] rounded-lg' />}
            <div className='flex flex-row gap-4'>
                {product.images.map((image,index)=>(
                    <img onClick={()=>setShownImg(index)} src={image.image} alt="image" className='sm:w-[25%] w-[35%] rounded-lg cursor-pointer hover:opacity-80' key={index} />
                ))}
            </div>
        </div>
        {/* text */}
